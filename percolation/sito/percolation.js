...
var Percolation = function(n) {
  ...
  this.bottom = (n * n) + 1;
  this.openSites = 0;
  this.canvas = new Canvas("table", n);
}
...
Percolation.prototype = {
  ...
  colorFull: function() {
    this.canvas.blue();
    for (var row = 1; row < this.n + 1; row++) {
      for (var col = 1; col < this.n + 1; col++) {
        if (this.unionFindWithoutBottom.connected(this.equivalent(row, col), this.top)) {
          this.canvas.open(row, col);
        }
      }
    }
...
var percolation = new Percolation(60);
